...
export function createReviewApp(root: string) {
  ...
  app.post('/api/upload', async (request, reply) => {
    ...
    const collection = request.headers['x-portrait-collection']
    try { return reply.code(201).send(await store.ingest(request.body, decodeURIComponent(name),
      typeof collection === 'string' && collection ? collection as PortraitCollection : undefined)) }
    catch (error) { return reply.code(400).send({ error: error instanceof Error ? error.message : 'Upload failed' }) }
  })
  app.post<{ Body: { ids: string[]; collection: PortraitCollection | null } }>('/api/collections', async (request, reply) => {
    try { return await store.assignCollection(request.body?.ids, request.body?.collection) }
    catch (error) { return reply.code(400).send({ error: error instanceof Error ? error.message : 'Invalid collection' }) }
  })
