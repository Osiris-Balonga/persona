...
const africaReviewedContexts = {
  ...
  CF: { pools: [{ locale: 'cf_CF', tier: 'local', weight: 1 }], fallback: 'local' },
  ...
  DJ: { pools: [{ locale: 'dj_DJ', tier: 'local', weight: 1 }], fallback: 'local' },
  EG: { pools: [{ locale: 'eg_EG', tier: 'local', weight: 1 }], fallback: 'local' },
  EH: { pools: [{ locale: 'eh_EH', tier: 'local', weight: 1 }], fallback: 'local' },
  ER: { pools: [{ locale: 'er_ER', tier: 'local', weight: 1 }], fallback: 'local' },
  ...
  GW: { pools: [{ locale: 'gw_GW', tier: 'local', weight: 1 }], fallback: 'local' },
  KE: { pools: [{ locale: 'ke_KE', tier: 'local', weight: 1 }], fallback: 'local' },
  KM: { pools: [{ locale: 'km_KM', tier: 'local', weight: 1 }], fallback: 'local' },
  LR: { pools: [{ locale: 'lr_LR', tier: 'local', weight: 1 }], fallback: 'local' },
  LS: { pools: [{ locale: 'ls_LS', tier: 'local', weight: 1 }], fallback: 'local' },
  LY: { pools: [{ locale: 'ly_LY', tier: 'local', weight: 1 }], fallback: 'local' },
  ...
  MU: { pools: [{ locale: 'mu_MU', tier: 'local', weight: 1 }], fallback: 'local' },
  MZ: { pools: [{ locale: 'mz_MZ', tier: 'local', weight: 1 }], fallback: 'local' },
  MR: { pools: [{ locale: 'mr_MR', tier: 'local', weight: 1 }], fallback: 'local' },
  NA: { pools: [{ locale: 'na_NA', tier: 'local', weight: 1 }], fallback: 'local' },
  NE: { pools: [{ locale: 'ne_NE', tier: 'local', weight: 1 }], fallback: 'local' },
  ...
  SD: { pools: [{ locale: 'sd_SD', tier: 'local', weight: 1 }], fallback: 'local' },
  SH: { pools: [{ locale: 'sh_SH', tier: 'local', weight: 1 }], fallback: 'local' },
  SL: { pools: [{ locale: 'sl_SL', tier: 'local', weight: 1 }], fallback: 'local' },
  SN: { pools: [{ locale: 'sn_SN', tier: 'local', weight: 1 }], fallback: 'local' },
  SO: { pools: [{ locale: 'so_SO', tier: 'local', weight: 1 }], fallback: 'local' },
  SS: { pools: [{ locale: 'ss_SS', tier: 'local', weight: 1 }], fallback: 'local' },
  ST: { pools: [{ locale: 'st_ST', tier: 'local', weight: 1 }], fallback: 'local' },
  SZ: { pools: [{ locale: 'sz_SZ', tier: 'local', weight: 1 }], fallback: 'local' },
  TD: { pools: [{ locale: 'td_TD', tier: 'local', weight: 1 }], fallback: 'local' },
  ...
  YT: { pools: [{ locale: 'yt_YT', tier: 'local', weight: 1 }], fallback: 'local' },
...
export function selectName(country: string, gender: 'male' | 'female', key: string) {
  ...
  if (isEuropeIslandCountry(country)) {
    const names = europeIslandNames[country]
    const firstName = names[gender][keyPart(key, 12) % names[gender].length]
    const lastName = names.family[keyPart(key, 24) % names.family.length]
    return { firstName, lastName, fullName: `${firstName} ${lastName}`, locale: selected.locale, fallback: selected.tier }
  }
  if (isAsiaReviewedCountry(country)) {
    const names = asiaReviewedNames[country]
    const firstName = names[gender][keyPart(key, 12) % names[gender].length]
    const lastName = names.family[keyPart(key, 24) % names.family.length]
    return { firstName, lastName, fullName: `${firstName} ${lastName}`, locale: selected.locale, fallback: selected.tier }
  }
  ...
  if (isAsiaCentralCountry(country)) {
    const names = asiaCentralNames[country]
    const firstName = names[gender][keyPart(key, 12) % names[gender].length]
    const family = gender === 'female' ? names.familyFemale : names.familyMale
    const lastName = family[keyPart(key, 24) % family.length]
    return { firstName, lastName, fullName: `${firstName} ${lastName}`, locale: selected.locale, fallback: selected.tier }
  }
  if (isAsiaAdditionalCountry(country)) {
    const names = asiaAdditionalNames[country]
    const firstName = names[gender][keyPart(key, 12) % names[gender].length]
    const lastName = names.family[keyPart(key, 24) % names.family.length]
    return { firstName, lastName, fullName: `${firstName} ${lastName}`, locale: selected.locale, fallback: selected.tier }
  }
