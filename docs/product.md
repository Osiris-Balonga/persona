...
| `email`, `phone` | Email at `example.test`; reserved-range or local-format phone string, with `null` when no format is available. Local-format values may be assigned. |
